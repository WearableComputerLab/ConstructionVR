...
public class UserStudyInterface : MonoBehaviour
{
    ...
    [SerializeField] private float parameterVariation = 0.5f; // % variation around base value
    ...
    void UpdateEnvironmentParameters()
    {
        if (useVerySubtleChanges)
        {
            ...
            currentHumidity += Random.Range(-0.01f, 0.01f);
        ...
        if (humidityText != null)
            humidityText.text = $"Humidity: {currentHumidity:F2}%";

        if (temperatureText != null)
            temperatureText.text = $"Temperature: {currentTemperature:F2}°C";
    }
